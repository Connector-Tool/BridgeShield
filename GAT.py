...
def train(data_path: str, model_args: dict, **kwargs):
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(dataset)):
        ...
        train_loader = DataLoader(train_subset, batch_size=kwargs.get('batch_size', 128), shuffle=True, num_workers=4)
        valid_loader = DataLoader(valid_subset, batch_size=kwargs.get('batch_size', 128), shuffle=False, num_workers=4)
        ...
        with torch.no_grad():
            ...
            save_classification_report("GAT", report_dict, "./result/GAT.csv")
            model.eval()
