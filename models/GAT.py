import torch.nn
from torch_geometric.nn import Linear, global_mean_pool, MLP, GATv2Conv, BatchNorm, global_max_pool


class GAT(torch.nn.Module):
    def __init__(
            self,
            hidden_channels: int,
            out_channels: int,
            num_layers: int,
            num_heads: int,
            metadata: tuple,
            **kwargs
    ):
        super().__init__()
        self.node_lins = torch.nn.ModuleDict()
        for node_type in metadata[0]:
            self.node_lins[node_type] = MLP(
                in_channels=-1,
                hidden_channels=hidden_channels,
                out_channels=hidden_channels,
                num_layers=2,
                norm=None,
            )

        self.edge_lins = torch.nn.ModuleDict()
        for edge_type in metadata[1]:
            edge_type = '__'.join(edge_type)
            self.edge_lins[edge_type] = MLP(
                in_channels=-1,
                hidden_channels=hidden_channels,
                out_channels=hidden_channels,
                num_layers=2,
                norm=None,
            )

        self.convs = torch.nn.ModuleList()
        self.bns = torch.nn.ModuleList()
        for _ in range(num_layers):
            self.convs.append(GATv2Conv(
                in_channels=hidden_channels,
                out_channels=hidden_channels // num_heads,
                edge_dim=hidden_channels,
                heads=num_heads,
            ))
            self.bns.append(BatchNorm(in_channels=hidden_channels))

        self.out_lin = Linear(hidden_channels, out_channels)

    def forward(self, data, **kwargs):
        node_types, edge_types = data.metadata()
        for node_type in node_types:
            data[node_type].x  = data[node_type].x.float()
            x = data[node_type].x
            data[node_type].x = self.node_lins[node_type](x)

        for edge_type in edge_types:
            edge_attr = data[edge_type].edge_attr
            data[edge_type].edge_attr = self.edge_lins['__'.join(edge_type)](edge_attr)

        data = data.to_homogeneous()
        for i, conv in enumerate(self.convs):
            data.x = conv(
                x=data.x,
                edge_index=data.edge_index,
                edge_attr=data.edge_attr,
            )
            data.x = self.bns[i](data.x)

        # emb = global_mean_pool(data.x, data.batch)
        emb = global_max_pool(data.x, data.batch)
        return self.out_lin(emb)
